...
from supply_program_engine.models import PipelineEntityView, EventType
...
def build_pipeline_state() -> Dict[str, PipelineEntityView]:
    ...
    for rec in ledger.read():
        ...
        ts = rec.get("ts")  # your ledger writes timestamp field; if named differently update here
        cid = rec.get("correlation_id")

        # Ensure entity view exists
        ...
        if et == EventType.CANDIDATE_INGESTED.value:
            # candidate payload contains company fields
            ...
            view.estimated_containers_per_month = int(payload.get("estimated_containers_per_month", view.estimated_containers_per_month) or 0)
...
def entity_timeline(entity_id: str) -> List[dict]:
    """
    Returns all events for an entity_id in ledger order.
    """
    out: List[dict] = []
    for rec in ledger.read():
